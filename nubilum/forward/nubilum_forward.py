...
class InstanceWrappedModel(torch.nn.Module):
    ...
    def forward(self,
        ...
        """
        Calculates only the logits for the points whose instance labels is the label of interest. It returns
        a tensor with the logits of the instance points for each class.
        ...
        if output_scores_key_name != None:
...
class PointWrappedModel(torch.nn.Module):
    ...
    def forward(self, point_coords: Tensor, point_colors: Tensor, point_of_interest: int, output_scores_key_name: str = None) -> Tensor:
        """
...
class SummarizedWrappedModel(torch.nn.Module):
    ...
    point. For more details, check https://captum.ai/tutorials/Segmentation_Interpret#Interpreting-with-Captum
    """
    def __init__(self, model):
        super().__init__()
        self.model = model

    def forward(self, point_coords: Tensor, point_colors: Tensor, output_scores_key_name: str = None) -> Tensor:
        """
